/**
 * sidepanel.js — RecruitFlow side panel controller
 * Loaded as an extension page script (Chrome Side Panel API).
 * All Chrome storage accessed directly; AI calls go through background.js.
 * DOM access to LinkedIn is done via message passing to content.js.
 */
(function () {
  'use strict';
  ...
  // ── Profile reading (via message to content.js — side panel has no LinkedIn DOM) ──
  async function readProfileFromDOM() {
    return (await sendToActiveTab({ type: 'GET_PROFILE' })) || { name: '', role: '', company: '', location: '', profileUrl: '' };
  }

  // ── LinkedIn message send (via message to content.js) ──────────────────────
  async function sendLinkedInMessage(text) {
    const result = await sendToActiveTab({ type: 'SEND_LINKEDIN_MESSAGE', message: text });
  ...
  function updateProfileBanner(profile) {
    ...
    const isProfilePage = /linkedin\.com\/in\//.test(profile?.profileUrl || '');
  ...
  function wireMessageTab() {
    ...
    // Re-read profile — ask content.js for the current tab's profile
    document.getElementById('rf-reread-btn')?.addEventListener('click', async () => {
      const profile = await readProfileFromDOM();
  ...
  // Resolve a city name to LinkedIn's internal geo ID via content.js
  // (content.js runs on LinkedIn and has access to the session cookies).
  async function resolveLinkedInGeoId(cityName) {
    const result = await sendToActiveTab({ type: 'RESOLVE_GEO_ID', cityName });
    return result?.geoId || '';
  }
  ...
  async function initMain() {
    ...
    // Read profile from active tab so templates fill correctly
    currentProfile = await readProfileFromDOM();
  ...
  async function init() {
    // Fetch current tab's profile when panel opens
    sendToActiveTab({ type: 'GET_PROFILE' }).then(profile => {
